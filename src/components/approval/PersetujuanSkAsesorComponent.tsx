...
import {
    ResponseSkAsesorForWarek,
    ResponseSkAsesorForWarekValue,
} from '@/types/PenunjukanAsesor'
import {
    getFileSkAsesorBlobByNamafile,
    getSkAsesorWarekPagination,
    setPersetujuanSkAsesor,
} from '@/services/Approval/ApprovalSkAsesorService'
...
export default function PersetujuanSkAsesorComponent() {
    ...
    const [data, setData] = React.useState<ResponseSkAsesorForWarek[]>([])
    const [dataSelected, setDataSelected] =
        React.useState<ResponseSkAsesorForWarek>(ResponseSkAsesorForWarekValue)
    ...
    React.useEffect(() => {
        ...
        getSkAsesorWarekPagination(
            ...
            .catch(() => {
                setLoading(false)
                toast.error('Gagal memuat daftar SK penugasan asesor')
            })
    ...
    const columns: ColumnDef<ResponseSkAsesorForWarek>[] = [
        {
            accessorKey: 'NomorSk',
            header: 'Nomor SK',
        ...
        },
        {
            accessorKey: 'NamaSk',
            header: 'Nama SK',
            cell: ({ row }) => <div>{row.getValue('NamaSk')}</div>,
        },
        {
            accessorKey: 'TahunSk',
            header: 'Tahun',
            cell: ({ row }) => <div>{row.getValue('TahunSk')}</div>,
        },
        {
            accessorKey: 'JumlahAsesor',
            header: 'Jumlah Asesor',
            cell: ({ row }) => <div>{row.getValue('JumlahAsesor')}</div>,
        },
        {
            id: 'actions',
            enableHiding: false,
                        ...
                        <TableBody>
                            {table.getRowModel().rows?.length ? (
                                ...
                                <TableRow>
                                    ...
                                    >
                                        Tidak ada SK yang menunggu persetujuan.
                                    </TableCell>
                                </TableRow>
                            )}
                        </TableBody>
...
function SheetPersetujuanSk({
    ...
    loading: boolean
    dataSelected: ResponseSkAsesorForWarek
    ...
    const openPreviewSk = async () => {
        try {
            const url = await getFileSkAsesorBlobByNamafile(
    ...
    return (
        <React.Fragment>
            <Sheet open={openDialog} onOpenChange={setOpenDialog}>
                ...
                >
                    <SheetHeader>
                        <SheetTitle>Persetujuan SK Penugasan Asesor</SheetTitle>
                        <SheetDescription>
                            Asesor baru dapat ditunjuk untuk asesmen RPL setelah
                            SK penugasannya disetujui.
                        </SheetDescription>
                    </SheetHeader>
                        ...
                        <div className="container mx-auto">
                            <div className="grid grid-cols-1 gap-3">
                                ...
                                <h5 className="text-center">
                                    Asesor yang Dicakup ({
                                        dataSelected.JumlahAsesor
                                    })
                                </h5>
                                <div className="flex flex-wrap gap-2">
                                    {dataSelected.Asesor.map((nama, idx) => (
