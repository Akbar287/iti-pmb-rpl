...
import {
    KabupatenFormSkemaValidation,
    KabupatenFormValidation,
} from '@/validation/AreaFormValidation'
import {
    deleteKabupaten,
    getKabupatenPagination,
    setKabupaten,
    updateKabupaten,
} from '@/services/AreaServices'

const KabupatenComponent = ({
    kabupatenDataServer,
}: {
    kabupatenDataServer: {
        CountryId: string
    ...
    const form = useForm<KabupatenFormValidation>({
        resolver: zodResolver(KabupatenFormSkemaValidation),
        defaultValues: {
            ProvinsiId: '',
            KabupatenId: '',
            Nama: '',
    ...
    const onSubmit = async (data: KabupatenFormValidation) => {
        setLoading(true)

        if (titleDialog === 'Ubah Kabupaten') {
            await updateKabupaten({
                ...
                .then((res) => {
                    toast('Data Kabupaten berhasil diubah')
                    let idx = dataKabupaten.findIndex(
                        (r) => r.KabupatenId === data.KabupatenId
                    )
                    setDataKabupaten(
                        replaceItemAtIndex(dataKabupaten, idx, res)
                    )
                    setOpenDialog(false)
                    setLoading(false)
                })
                .catch((err) => {
                    toast('Data Kabupaten gagal diubah. Error: ' + err)
                    setLoading(false)
                })
        } else {
            await setKabupaten({
                ...
                .then((res) => {
                    toast('Data Kabupaten berhasil ditambah')
                    setDataKabupaten([...dataKabupaten, res])
                    setLoading(false)
                    setOpenDialog(false)
                })
                .catch((err) => {
                    toast('Data Kabupaten gagal ditambah. Error: ' + err)
                    setLoading(false)
                })
        }
    }
    ...
    const hapusData = (jd: Kabupaten) => {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                deleteKabupaten(jd.KabupatenId).then(() => {
                    setDataKabupaten(
                        dataKabupaten.filter(
                            (r) => r.KabupatenId !== jd.KabupatenId
                        )
                    )
                    Swal.fire({
                        title: 'Terhapus!',
    ...
    return (
        <div className="w-full">
            <Card className="bg-gray-50 shadow-md dark:bg-gray-800">
                <CardHeader>
                    <CardTitle>
                        <h1 className="text-2xl">Kabupaten</h1>
                    </CardTitle>
                    <CardDescription>Kabupaten</CardDescription>
                </CardHeader>
                <CardContent>
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                        <div className="w-full">
                            ...
                            <Select
                                ...
                                onValueChange={(e) => {
                                    let temp = kabupatenDataServer.find(
                                        (x) => x.CountryId === e
                                    )
                                    setselectedData({
                                        CountryId: e,
...
export default KabupatenComponent
...
export function SheetManageData({
    ...
    loading: boolean
    onSubmit: (data: KabupatenFormValidation) => void
    form: UseFormReturn<KabupatenFormValidation>
    titleDialog: string
    selectedData: {
        ProvinsiId: string
        NamaProvinsi: string
