...
const Community = ({
    ...
    const photos = data
        ? data.map((x) => ({
              id: x.SettingCommunityId,
              src:
                  process.env.NEXT_PUBLIC_API_BASE_URL +
                  `/api/img?_t=_c&_id=${x.SettingCommunityId}`,
              alt: x.SettingCommunityId,
              caption: x.Title,
          }))
        : [
              {
              ...
              },
              {
                  ...
                  caption:
                      'Diskusi seminar kecil mendorong kolaborasi dan inovasi',
              },
              {
                  id: 4,
    ...
    const testimonials = testimony
        ? testimony.map((z) => ({
              quote: z.Testimoni,
              name: z.Nama,
              position: z.JurusanTahun,
              image:
                  process.env.NEXT_PUBLIC_API_BASE_URL +
                  `/api/img?_t=_t&_id=${z.SettingTestimonyId}`,
          }))
        : [
              {
                  quote: 'Waktu saya di ITI adalah pengalaman yang mengubah hidup. Saya tidak hanya belajar dari para ahli di bidang saya, tetapi juga bertemu dengan teman seumur hidup dari seluruh dunia.',
                  name: 'Akbar Alamsyah',
                  position: 'Arsitektur, 2019',
                  image: 'https://randomuser.me/api/portraits/women/45.jpg',
              },
              {
                  quote: 'ITI memberikan saya kesempatan untuk mengeksplorasi minat saya di luar kurikulum akademis. Saya terlibat dalam berbagai organisasi mahasiswa yang memperkaya pengalaman saya.',
                  name: 'Diana Putri',
                  position: 'Teknik Elektro, 2020',
                  image: 'https://randomuser.me/api/portraits/men/32.jpg',
              },
              {
                  quote: 'Penelitian yang dilakukan di ITI sangat inovatif dan berdampak. Saya merasa terhormat dapat berkontribusi pada proyek yang memiliki potensi untuk mengubah dunia.',
                  name: 'Maulana Hakiem',
        ...
        <section id="community" className="container-padding container mx-auto">
            <div className="text-center mb-16">
                <h2 className="section-title mx-auto text-center">
                    {text ? text.CommunityText : 'Kenalan dengan Komunitas'}
                    <span className="block mx-auto mt-2 w-24 h-1 bg-primary"></span>
                </h2>
                <p className="section-subtitle mx-auto">
                    {text
                        ? text.CommunityDeskripsi
                        : 'Anda dapat menemukan lingkungan yang mendukung dan mendorong pertumbuhan pribadi dan akademis.'}
                </p>
            </div>
            ...
            <div className="mt-20">
                <div className="text-center mb-16">
                    <h2 className="section-title mx-auto text-center">
                        {text ? text.TestomoniText : 'Apa Kata Mereka ?'}
                        <span className="block mx-auto mt-2 w-24 h-1 bg-primary"></span>
                    </h2>
                    <p className="section-subtitle mx-auto">
                        {text
                            ? text.TestomoniDeskripsi
                            : 'Mereka adalah alumni ITI yang sudah berkarya untuk Indonesia'}
                    </p>
                </div>
                <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
                    {testimonials.map((testimonial, index) => (
                        <div
                            key={index}
                            className="bg-white dark:bg-gray-800 p-8 rounded-lg shadow-md border-t-4 border-primary"
                        >
