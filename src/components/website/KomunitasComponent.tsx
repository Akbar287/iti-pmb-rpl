...
import {
    SettingCommunityFormValidation,
    SettingCommunitySkemaValidasi,
...
const KomunitasComponent = ({
    ...
    const [dataKomunitas, setDataKomunitas] = React.useState<
        SettingCommunityTypes[]
    >([])
    ...
    const form = useForm<SettingCommunityFormValidation>({
        resolver: zodResolver(SettingCommunitySkemaValidasi),
        defaultValues: {
            Gambar: null,
            Title: '',
            SettingMainPageId: '',
    ...
    const onSubmit = async (data: SettingCommunityFormValidation) => {
        if (titleDialog === 'Ubah Komunitas' && data.Gambar) {
            await updateKomunitas(data.Gambar, {
                Title: data.Title,
                SettingCommunityId: data.SettingCommunityId,
                SettingMainPageId: data.SettingMainPageId,
            })
                .then((res) => {
                    toast('Data Komunitas berhasil diubah')
                    let idx = dataKomunitas.findIndex(
                        (r) => r.SettingCommunityId === data.SettingCommunityId
                    )
                    setDataKomunitas(
                        replaceItemAtIndex(dataKomunitas, idx, res)
                    )
                    setOpenDialog(false)
                    setLoading(false)
                })
                .catch((err) => {
                    toast('Data Komunitas gagal diubah. Error: ' + err)
                    setLoading(false)
                })
        } else {
            if (data.Gambar) {
                await setKomunitas(data.Gambar, {
                    Title: data.Title,
                    SettingCommunityId: data.SettingCommunityId,
                    SettingMainPageId: data.SettingMainPageId,
                })
                    .then((res) => {
                        toast('Data Komunitas berhasil ditambah')
                        setDataKomunitas([...dataKomunitas, res])
                        setLoading(false)
                        setOpenDialog(false)
                    })
                    .catch((err) => {
                        toast('Data Komunitas gagal ditambah. Error: ' + err)
                        setLoading(false)
                    })
            }
        }
    ...
    const buatData = () => {
        ...
        setTitleDialog('Tambah Komunitas')
        setOpenDialog(true)
    }
    const ubahData = async (jd: SettingCommunityTypes) => {
        form.setValue('Title', jd.Title)
        form.setValue('Gambar', null)
        form.setValue('SettingCommunityId', jd.SettingCommunityId)
        form.setValue('SettingMainPageId', jd.SettingMainPageId)
        urlToFile(
            process.env.NEXT_PUBLIC_API_BASE_URL +
                '/api/img?_t=_c&_id=' +
                jd.SettingCommunityId
        )
            .then((res) => form.setValue('Gambar', res))
            .catch((err) => toast('Gagal Ambil data Gambar'))
        setTitleDialog('Ubah Komunitas')
        setOpenDialog(true)
    }
    const hapusData = (jd: SettingCommunityTypes) => {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                deleteKomunitas(jd.SettingCommunityId).then(() => {
                    setDataKomunitas(
                        dataKomunitas.filter(
                            (r) =>
                                r.SettingCommunityId !== jd.SettingCommunityId
                        )
                    )
                    Swal.fire({
                        title: 'Terhapus!',
    ...
    const columns: ColumnDef<SettingCommunityTypes>[] = [
        {
            accessorKey: 'Gambar',
            header: 'Gambar',
            cell: ({ row }) => (
                <img
                    src={
                        process.env.NEXT_PUBLIC_API_BASE_URL +
                        '/api/img?_t=_c&_id=' +
                        row.original.SettingCommunityId
                    }
                    alt={row.original.SettingCommunityId}
                />
            ),
        },
        {
    ...
    return (
        <div className="w-full">
            <Card className="shadow-md">
                <CardHeader>
                    <CardTitle>
                        <h1 className="text-2xl">Komunitas</h1>
                    </CardTitle>
                    <CardDescription>Gambar Komunitas</CardDescription>
                </CardHeader>
                <CardContent>
...
export function SheetManageData({
    ...
    loading: boolean
    onSubmit: (data: SettingCommunityFormValidation) => void
    form: UseFormReturn<SettingCommunityFormValidation>
    titleDialog: string
